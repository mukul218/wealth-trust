...
<head>
  ...
  <!-- Base + SIP UI styles (reuse the SIP look) -->
  ...
  <script type="application/ld+json">
    {
      ...
      ",
      "url": "https://www.wealthtrustcap.com/retirement-calculator"
    }
  </script>
        ...
        <div class="card sip-card shadow-sm">
          <div class="card-body p-4">
            <div class="d-flex align-items-center mb-3">
              <h2 class="h4 mb-0 me-2">Retirement <span class="text-success">Calculator</span></h2>
              <span class="badge bg-light text-secondary border ms-2">Goal @ Retirement</span>
            </div>
            <p class="text-muted small mb-4">
              Calculate the monthly SIP needed to reach your retirement corpus, considering inflation
            ...
            <form class="calc-form" data-calculator="retirement">
              ...
              <div class="d-flex gap-2 align-items-center">
                ...
                <button class="btn btn-outline-secondary px-4" type="reset" id="retirementReset">Reset</button>
              </div>
            </form>
        ...
        <div class="card shadow-sm h-100">
          <div class="card-body p-4">
            ...
            <div class="mb-4" style="max-width: 300px; margin: auto;">
              <canvas id="retirementPieCanvas" height="200"></canvas>
            </div>
    ...
    $(function() {
      ...
      setupCalculatorForm(".calc-form", {
        onSuccess: function(d) {
          // Using your response structure:
          // { current_age, retirement_age, wealth_amount, inflation_rate, expected_return, savings_amount,
          //   target_wealth, target_savings, target_amount, years, monthly_savings, invested_amount, total_earnings }

          const currentAge = Number(d.current_age ?? $("#current_age").val() ?? 0);
          const retireAge = Number(d.retirement_age ?? $("#retirement_age").val() ?? 0);
          const years = Number(d.years ?? Math.max(0, retireAge - currentAge));
          const months = years * 12;

          const reqSip = Number(d.monthly_savings ?? 0);
          const targetWealth = Number(d.target_wealth ?? 0);
          const finalCorpus = Number(d.target_amount ?? 0);
          const invested = Number(d.invested_amount ?? 0);
          const earnings = Number(d.total_earnings ?? 0);
          const addlSavings = Number(d.target_savings ?? 0);
          ...
          $("#finalCorpus").text("₹" + formatINR(finalCorpus));
          ...
          // Pie chart: Invested vs Earnings
          const pieData = {
          ...
          const ctx = document.getElementById("retirementPieCanvas").getContext("2d");
          if (retirementPieChart instanceof Chart) {
            retirementPieChart.data = pieData;
            retirementPieChart.update();
          } else {
            retirementPieChart = new Chart(ctx, {
              type: "doughnut",
              data: pieData,
              options: {
                responsive: true,
      ...
      // Reset cleanup
      $("#retirementReset").on("click", function() {
        $(".result-box").addClass("d-none");
        if (retirementPieChart instanceof Chart) {
          retirementPieChart.destroy();
          retirementPieChart = null;
        }
      });
